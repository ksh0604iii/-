...
def get_report(ticker):
    try:
        ...
        if per < 10: score += 10 # PER 점수
        if pbr < 1.0: score += 5  # PBR 점수
        if div > 3: score += 5    # 배당 점수
...
t1, t2, t3 = st.tabs(["🎯 실시간 분석", "🚀 A-B등급 발굴", "📰 글로벌 뉴스 수집"])
    ...
    if st.button("모든 뉴스 새로고침"):
        for kw in KEYWORDS:
            ...
            for i in n_res.json().get('items', []):
                st.markdown(f"📍 [네이버] [{i['title'].replace('<b>','').replace('</b>','')}]({i['link']})")
            # 외신 수집 및 번역
            try:
